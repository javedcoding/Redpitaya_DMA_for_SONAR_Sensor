...
#define V_SONIC_WAVE                    (float)(343.2)  // [m/s] velocity of ultrasonic wave in air
...
#define ADC_SAMPLE_FREQUENCY            ( ADC_MAX_SAMPLE_FREQUENCY / ADC_SAMPLE_DECIMATION )            // 125MHz/64 --> 1.953125MHz
...
#define ADC_START_DELAY_US              (uint32_t)( 0.30 * 2 * 1e6 / V_SONIC_WAVE )                     // [µs] --> 2 * 0,30 m / 343,2 m/s = 1.748 ms or 1748µs 
#define ADC_BUFFER_DELAY_US			    (uint32_t)(( ADC_BUFFER_SIZE * ADC_SAMPLE_TIME_NS ) / (1e3))	// [µs] --> (16.384 * 512 ns ) / 1000 = 8.388 µs In current version not used
#define ADC_MID_US					    (ADC_START_DELAY_US + ( ADC_BUFFER_DELAY_US / 2 ))              // In Current version not used
...
#define SW_VERSION                      2.00                // This is the current software version
...
/* Typedefs for data structs: Header and data 
    [C]-Denotes its constant type and do not change
    [V]-Denotes its variable type and changes with every data taking
*/
typedef struct
{
