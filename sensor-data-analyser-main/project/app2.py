# -*- coding: utf-8 -*-
"""
Created on Sun Aug 11 17:24:25 2024
...
class RabbitMQClient:
    def __init__(self, host='localhost', queue='sensor_data'):
...
class Worker(QRunnable):
    ...
    Worker thread
    '''
    def __init__(self, func_is_button_checked, rp_sensor, rabbitmq_client, *args, **kwargs) -> None:
        super().__init__()
        self.func_is_button_checked = func_is_button_checked
        self.rp_sensor = rp_sensor
    ...
    def run(self):
        ...
        while self.func_is_button_checked(*self.args, **self.kwargs) and self.is_running:
            try:
                ...
                self.rabbitmq_client.publish_message(result.to_json())
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.rabbitmq_client = RabbitMQClient(host="192.168.128.2", queue="sensor_data_queue")
    ...
    def realtime_checkbox_handler(self, state):
        ...
        if state == Qt.CheckState.Checked.value:
            ...
            self.worker.signals.signals_count_updated.connect(self.update_save_button_state)
    ...
    def realtime_checkbox_handler(self, state):
        if state == Qt.CheckState.Checked.value:
            ...
            self.show_region_chkbox.setDisabled(True)
            self.confirm_region_btn.setDisabled(True)
            self.worker = Worker(self.func_is_realtime_checked, self.rp_sensor, self.rabbitmq_client)
            self.worker.signals.result.connect(self.plot_adc_data)
            self.worker.signals.signals_count_updated.connect(self.update_save_button_state)
            self.threadpool.start(self.worker)
        else:
    ...
    def closeEvent(self, event):
        if self.worker:
            self.worker.stop()
            self.threadpool.waitForDone()
        self.rabbitmq_client.close_connection()  # Close the RabbitMQ connection
        event.accept()
