...
typedef struct
{
	float HeaderLength; //this define the header length for client packet reading
    float DataLength; //this defines the data length of the current sent packet in bytes (25000*4)+(6*4)
    float TotalDataBlocks; //this gives how many datablocks per signal will be sent
	float DataBlockNumber; //this gives the current sent data block number
	float SampleFrequency; //sampling frequency of the ultrasonic signal
    float CurrentTime; //this gives the current time inside redpitaya after the udp connection when program starts for sync
	uint16_t p_data[READ_DATA_SIZE]; //this holds the actual ADC data
}Header_t;
